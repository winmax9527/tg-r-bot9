...
router = APIRouter()
# Bot 6 的令牌环境变量
BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN_6")
...
if BOT_TOKEN:
    ...
    logger.warning("Bot 6 (短链接生成) Token 未配置 (TELEGRAM_BOT_TOKEN_6)。请设置环境变量。")
...
async def link_command_handler(update: Update, context):
    ...
    await update.message.reply_text("Bot 6 正在为您生成短链接，请稍候...")
...
async def handle_webhook(update_data: dict):
    ...
    try:
        ...
        logger.error(f"Bot 6 处理 Webhook 错误: {e}", exc_info=True)
...
async def webhook_handler(update_data: dict):
    ...
    if not BOT_TOKEN:
        raise HTTPException(status_code=500, detail="Bot 6 Token 未配置。")
